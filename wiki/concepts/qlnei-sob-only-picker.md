---
type: concept
title: qlnei — sob-only BO picker
description: single-objective `qLogNoisyExpectedImprovement` picker (sob only, no
  Pareto-HV); stamps AUTORESEARCH_NO_RUN1B to drop the DS-off stage (40% wall-clock
  saving); wired 2026-06-08 after 5-campaign saturation at sob≈3.89
status: active
...
> [foils-cloud-calo-zero-inf](/incidents/foils-cloud-calo-zero-inf.md)). Also note the stamp's stage-drop saves
> no wall-clock for foilsflash. Relevant if the unexplored foilsflash sob
> corner (best 3.77 vs foils' 3.91, see [bo-foilsflash](/projects/bo-foilsflash.md)) is ever pushed.
...
> give calo genuine ~17× spread ([bo-foils](/projects/bo-foils.md)), the non-dominated front
...
- `_qlnei_picks(model, X, bounds, q, round_idx)`: builds `qLogNoisyExpectedImprovement(model, X_baseline=X)`, optimizes via `optimize_acqf` with same Sobol seed convention as qnehvi (`seed = 42 ^ round_idx` — see [botorch-predict-seed-pow-vs-xor](/incidents/botorch-predict-seed-pow-vs-xor.md)).
...
- If qlnei breaks the sob ceiling, document in [bo-foils](/projects/bo-foils.md) log; if not, document why saturation is genuine (see also [gp-cloud-rendering](/concepts/gp-cloud-rendering.md)).
