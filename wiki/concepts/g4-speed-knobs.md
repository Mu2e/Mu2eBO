...
## Key facts
- **Our chain's G4 stages are SINGLE-threaded** (Production PR#449, merged 2025-08-28 — predates every pinned Musing: Run1Bak 2026-05-13, Run1Bap, Run1Baq): `#include Production/JobConfig/common/MT.fcl` was removed from POT.fcl and all five pileup resamplers (incl. our MuBeamResampler + EleBeamResampler) after issue Production#222 showed the MT win is fake when G4 is not the dominant module (MuStopPileup: 2.3 s/ev in the serialized legacy ResamplingMixer vs 0.2 s/ev G4 — 2 threads ≈ same wall clock at 2× CPU). `MT.fcl` itself survives (Mu2eG4MT, 2 threads/2 schedules) but only STM/wideband/validation fcls include it — nothing our stages run. Requesting >1 CPU for our grid jobs buys nothing.
- **Why MT is 'broken' (it isn't, mechanically — it's mis-shaped for art)**: `Mu2eG4MT` itself is fine and per-event reseeded (`Mu2eG4WorkerRunManager.cc:283` hashes run/subrun/event+salt into `G4Random::setTheSeeds`, so G4 output is reproducible). Three real defects, all upstream/downstream of G4: (1) art serializes every legacy module, so with a mixer-dominated schedule 2 threads ≈ same wall clock at 2× CPU (Production#222); (2) any seeded legacy module DOWNSTREAM of Mu2eG4MT sees events in scheduler-dependent order — its random chain, and the job output, is irreproducible (Offline#849, OPEN since 2022; proposed fix = per-event EventID seeding for all modules, measured cost ~0.25% CPU, never implemented); (3) the one Replicated generator, `PrimaryProtonGun`, had instances sharing one seed + scheduler-random event→instance matching (Offline#482: two identical 5-thread runs overlapped in only ~10% of output events) — 'fixed' by PR#747 reverting it to a serialized legacy EDProducer, and #744/#745 closed 2022-09-13 as 'no need for this module to run multi-threaded'. So MT survives only where G4 >90% of CPU and nothing seeded runs after it.
- **REPRODUCED LOCALLY 2026-08-26** (ceSimReco, Run1Bak, seed 8, 100 events, 2×2 pairs): single-threaded pair bit-identical (17/17 events written, g4 and digi hashes equal); Mu2eG4MT 2-thread pair DIVERGENT — 16 vs 12 events written, only 9 common, and on those 9 the `g4run` tracker StepPointMC hashes are IDENTICAL while the StrawDigi hashes differ 9/9 (TDC/TOT/PMP). Localizes the break exactly as Offline#849 predicts: G4's per-event reseed holds, the seeded legacy digitizers downstream break. BONUS bug: one MT run died at teardown with G4 fatal `G4CacheReference<V>::Destroy` "created in a thread ... deleted from another thread" (after the output file closed, so exit code lies about the data being fine). Mechanism (CORRECTED 2026-08-26 by gdb backtrace — the first-pass story blaming module-teardown thread-matching was wrong): the crash is at `__run_exit_handlers` on the MAIN art thread, `G4ThreadLocalSingleton<G4HadronicInteractionRegistry>` static dtor destroying worker-thread ParticleHP models; `G4Cache` ids are per-template-type counters against a THREAD-LOCAL store, and main's store is shorter than the worker-created ids. Full mechanism + fix: [g4cache001-exit-teardown-mt](/incidents/g4cache001-exit-teardown-mt.md). Repro harness: scratchpad mtrepro/ (4 fcl overlays + PyROOT pair-hasher).
- **FIX IMPLEMENTED + VALIDATED 2026-08-26** (the Offline#849 design, built here): `SeedService/inc/PerEventReseed.hh` — per-event `engine.setSeed(hash(run/subrun/event + module-class salt))`, the exact Mu2eG4WorkerRunManager recipe, gated on env `MU2E_RESEED_PER_EVENT` (default OFF — existing productions unchanged); wired into the 8 seeded modules downstream of g4run in ceSimReco (StrawDigisFromStrawGasSteps, CaloDigiMaker, CaloShowerROMaker, Crv{Photon,SiPMCharge,Waveforms}Generator, EventWindowMarkerProducer, ProtonBunchIntensityLogNormal — the last needed an `engine_` ref kept beside its `artURBG` wrapper). Validation (100 ev, seed 8): MT 2×2 pair went DIVERGENT (16 vs 12 events) to **bit-IDENTICAL** (15/15, g4+digi hashes equal); ST pair stayed identical. ST-vs-MT cross remains different (physics-equivalent; raw g4 tracker steps identical per event, divergence enters via unpersisted intermediate products — unpinned, benign). The G4Cache teardown fatal recurred (1-of-2 MT runs) — separate bug, root-caused + fixed 2026-08-26 ([g4cache001-exit-teardown-mt](/incidents/g4cache001-exit-teardown-mt.md)). Upstreamed as **draft PR Mu2e/Offline#1951** (2026-08-26, branch `oksuzian:per-event-reseed` off main — CaloMC pair re-anchored, main compile left to FNALbuild CI). Patch: `/exp/mu2e/app/users/oksuzian/Offline_mtfix_partial/per-event-reseed.patch` (9 files, +62/−1), partial-checkout build per [muse-backing-pattern](/external/muse-backing-pattern.md), 8 plugin .so at `Offline_mtfix_partial/build/al9-prof-e29-p094/Offline/lib/` (activate: prepend CET_PLUGIN_PATH + LD_LIBRARY_PATH, export the env var).
- **SCALE CAVEAT 2026-08-26 (10k events, 4 threads x 4 schedules)**: the per-event reseed is NOT fully closing MT determinism at scale — pair run: g4 StepPointMC hashes still identical (0/1738 differ; upstream + G4 chain deterministic even at 4x4), but StrawDigi hashes differ on 37/1738 common events (~2%) and the trigger-passed event sets differ (1756 vs 1753, 18/15 exclusive). Divergence still enters strictly between g4run and the digitizers. Prime suspect: cross-event state ABOVE the engine that `setSeed` does not clear — e.g. CLHEP `RandGauss`-style cached spare deviate (Box-Muller pair caching in the distribution object survives engine reseed; draw-parity then depends on event order). 100-event 2x2 pairs were bit-identical, consistent with a rare/parity-dependent leak. TODO before un-drafting PR#1951: audit the 8 modules' distribution objects (RandGauss vs RandGaussQ, RandPoisson state), clear distribution caches at reseed, re-run the 10k pair.
- **POT MT validation 2026-08-27** (potSimMT, 5000 ev pair, 8 threads x 8 schedules, Cache001 handler + reseed libs active): (a) **scaling near-ideal** — CPU 7023 s / wall 926 s = 7.6x on 8 schedules (95%), instantaneous ~793% CPU; POT has no Amdahl ceiling (the serial lane is a ceSimReco/reco-chain property — measured there: G4 34%, CaloShowerStepMaker 16%, 4x LoopHelixFit 22%, compress* 13%, StrawDigis 5%, and ALL legacy modules share ONE global serialization token). (b) **Cache001 clean at POT** — both runs exit 0, handler fired once each (the workload where Genser saw the fatal most). (c) **bit-reproducibility is NOT achievable for POT MT**: pair diverges on 79/5000 events (filter pass 3927 vs 3933) with GenParticles IDENTICAL 0/5000 and `nG4Tracks` differing (ev 1: 15184 vs 11264 tracks, cpu 15.2 vs 11.0 s, no overflow/step-kills) — G4 grows a different shower from the same primary and same per-event seed. This is G4-INTERNAL: the documented NeutronHP/ParticleHP MT reproducibility violation (G4 MT task force lists it as under investigation; G4 guarantees only "weak reproducibility"). Neutron-dominated ShieldingM POT showers expose it at ~1.6%/event; CE tracker products in ceSimReco masked it (g4 hashes there were identical — the ceSimReco digi divergence above is a SEPARATE art-side residual). Not fixable art-side; physics is statistically equivalent, only bit-level identity is lost. MT POT verdict: crash fixed, scaling excellent, but grid bit-reproducibility requires single-thread until G4 fixes HP.
