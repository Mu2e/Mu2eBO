...
| 8 | MCP server | Studies become problems. `stats` carries knob names, units and bounds, and objective names, directions and transforms. `list_problems` is generated by surrokit and stays as it is. |
...
```json
{
  ...
  "extra_metrics": [],
  "extra_columns": [{"name": "alpha", "expr": "alpha",                  "fmt": "{:.3f}"},
...
- `profiles` expand control points into `count` values. `kind` is required; only `lagrange` exists now, and it takes exactly 3 control points (a quadratic at u = 0, 0.5, 1, as today), clipped to `clip`. Control points may be knob names or expressions. Other degrees are a later `kind`.
...
**`extra_columns`** are row columns computed at scoring by an expression over the objective and extra-metric names, `derive.consts`, and the runtime values named in `leaderboard.context`. They exist so that a converted study keeps its board's exact column layout. foilspf's boards end in `alpha` and `obj`, where `obj = sob − alpha·flash`. Nothing optimizes them.
...
**Pending files** (`pending/*.tsv`) keep today's format in this design. Removing the vestigial `alpha` column goes with removing the `--alpha` flag, a later cleanup.
...
**Parity:** the existing `tests/golden_parity.py` is extended with the gates above. Picks are compared through the exact inputs handed to `surrokit.ask`, because the pick outputs are not bit-reproducible run to run (hybrid-picker incident).

**Changed on purpose in Phase A:** today's flash objective falls back to `flash_edep_per_event` when `flash_edep_per_pot` is missing. Those are different units, so the fallback is dropped: a missing `flash_edep_per_pot` now makes the evaluation fail instead of landing a row in the wrong units.
