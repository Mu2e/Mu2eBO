...
paginate: true
size: 16:9
footer: "FoilsFlash — foils cut tracker electron-flash per POT ~2.7× · Y. Oksuzian · 2026-07-11"
style: |
...
**Y. Oksuzian**
2026-07-11 · Mu2e — autoresearch / closed-loop BO
...
> **no S/√B cost**. Twelve **closed-loop BO campaigns** + a directed transplant (249 evals) map a
...
> **+68% flash** — and a Pareto-exploit round just priced near-ceiling 3.80 at only **+39%** (8.97×10⁻⁷).
...
<small>249 evals (foilsflash02–13 + transplant, green ◆ at S/√B 3.90; red ✚ = champion, below-left of the deployed-default gold ★: better on both axes), colored by `rOut_dn` (dominant knob). GP Pareto front (cyan) traces the low-flash/high-S/√B edge.</small>
...
- **best/worst ≈ 2.7×** in flash-per-POT across the 249 evals (GP fit R² ≈ 0.9).
...
transplant seeded the GP, the optimizer **self-found** this corner (rows 2–3); a GP-mean **Pareto-exploit
round** (`pareto_sob`, ff13) then cut its flash price to 8.97×10⁻⁷ (row 4, was ≥1.04×10⁻⁶ for 3.8-class S/√B).
...
- **Twelve closed-loop BO campaigns (249 evals)** converged on a floor-flash / high-S/√B corner; for
...
- **The S/√B ceiling is measured, and priced**: the transplant (`foilsflashSOBX01`) reproduces
  **3.90 vs 3.91 across two independent pipelines** at **+68% flash**; once it seeded the surrogate,
  the optimizer **self-reached 3.82** (was stuck at 3.77 for 7 campaigns). A GP-mean **Pareto-exploit
  round** (`pareto_sob`, foilsflash13) then repriced near-ceiling S/√B: **3.80 at 8.97×10⁻⁷ (+39% vs
  deployed, was ≥+61%)**, filling the empty 9–10×10⁻⁷ band with 3 new front points. Optimal upstream
  foil thickness localized to **~35–50 µm** (sub-20 µm buys nothing — widened-box probe).
...
- **Next:** 400-job flash confirmation of the new sob-corner point (`foilsflash13R00_02`; single-run
  flash σ ≈ 5%), then the **central-hole line** (`bo-foilshole`) — Edmonds' ~30% central-hole lever
  is 6× what the outer envelope bought.
